...
class ft_linear_regression(gestion_data):
    def __init__(self, gestion_data):
        ...
        self.normalized_price_values = self.normalized_price_values(self.price_values)
        ...
        self.line_km = self.calculate_line_km(self.km_values, self.price_values)
        self.line_price = self.calculate_line_price(self.teta1, self.teta0, self.line_km)
    ...
    def normalized_km_values(self, km_values):
        ...
        for value in km_values:
            normalized_km_values.append((value - km_values[idx_km_min]) / (km_values[idx_km_max] = km_values[idx_km_min]))
    ...
    def normalized_price_values(self, price_values):
        ...
        for value in price_values:
            normalized_price_values.append((value - price_values[idx_price_min]) / (price_values[idx_price_max] - price_values[idx_price_min]))
    ...
    def gradient_descent_LR(self, km_values, price_values, epochs, lr):
        ...
        for e in range(epochs):
            teta1 = teta1 - lr * self.teta1_grad(teta1, teta0, km_values, price_values)
            teta0 = teta0 - lr * self.teta0_grad(teta1, teta0, km_values, price_values)
        return teta1, teta0
